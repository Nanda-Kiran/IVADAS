...
def save_softpatch_model(run_save_path, dataset_name, coreset_list, args):
    """
    Save trained SoftPatch coresets using the built-in save_to_path method.
    This avoids pickling lambdas/FAISS and is compatible with load_from_path.
    """
    model_root = os.path.join(run_save_path, "models", dataset_name)
    os.makedirs(model_root, exist_ok=True)

    meta = {
        "dataset_name": dataset_name,
        "args": vars(args),
        "num_coresets": len(coreset_list),
    }

    # Save each coreset into its own subfolder
    for i, coreset in enumerate(coreset_list):
        coreset_dir = os.path.join(model_root, f"coreset_{i}")
        os.makedirs(coreset_dir, exist_ok=True)
        LOGGER.info(f"[SoftPatch] Saving coreset {i} to {coreset_dir}")
        coreset.save_to_path(coreset_dir)

    # Optional: save a tiny metadata file for convenience
    meta_path = os.path.join(model_root, "meta.pkl")
    with open(meta_path, "wb") as f:
        pickle.dump(meta, f, pickle.HIGHEST_PROTOCOL)

    LOGGER.info(f"[SoftPatch] Saved {len(coreset_list)} coresets to {model_root}")
    return model_root
...
def run(args):
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        with device_context:
            ...
            # 🔥 Save SoftPatch "weights" PatchCore-style
            save_softpatch_model(run_save_path, dataset_name, coreset_list, args)
        
            # (Optional) Plot example images.
            if args.save_segmentation_images:
                image_paths = [
                    x[2]
                    for x in dataloaders["testing"].dataset.dataset.data_to_iterate[
                        dataloaders["testing"].dataset.indices
                    ]
                ]
                mask_paths = [
                    x[3]
                ...
                def mask_transform(mask):
                    return (
                        dataloaders["testing"]
                        .dataset.dataset.transform_mask(mask)
                        .numpy()
                    )
